/* Creating Grid */ let mainGrid = document.querySelector(".main-grid");
...
createGrid(16);
/* Fill grid cells with color */ function colorFill(e1) {
    let gridCells = document.querySelectorAll(".grid-element");
    let targetColor = e1.target;
    if (targetColor.classList.contains("black-btn")) gridCells.forEach((cells)=>{
        cells.addEventListener("mouseover", (e)=>{
            e.target.style.backgroundColor = "black";
        });
    });
    else if (targetColor.classList.contains("rainbow-btn")) gridCells.forEach((cells)=>{
        cells.addEventListener("mouseover", (e)=>{
            e.target.style.backgroundColor = `hsl(${Math.random() * 360}, 100%, 50%)`;
        });
    });
    else if (targetColor.classList.contains("eraser-btn")) gridCells.forEach((cells)=>{
        cells.addEventListener("mouseover", (e)=>{
            e.target.style.backgroundColor = "hsl(0, 4%, 89%)";
        });
    });
}
/* Reset size of grid with user's input */ let gridSize = document.querySelector(".input-grid-size");
...
setButton.addEventListener("click", function resetGridSize() {
    ...
    if (input >= 2 && input <= 80) {
        ...
        mainGrid.innerHTML = '';
        createGrid(input);
    } else alert("Number shouldn't be lower than 2 and greater than 80. Please try again.");
});
